...
onMounted(() => {
  ...
  const computeShipParticle = (
    ...
    const waveSpeed = addControl('waveSpeed', 'Sea Wave Speed', 0, 5, 2)
    const windForce = addControl('windForce', 'Wind Force', 0, 20, 10)
    const rowSpeed = addControl('rowSpeed', 'Rowing Speed', 0, 10, 6)
    ...
    if (u < 0.4) {
      ...
      let billow = Math.sin(sx * 2) * 0.15 + Math.sin(sy * 1.5) * 0.15
      billow += Math.sin(time * waveSpeed + sy) * (windForce * 0.005)
      const cornerPull = Math.abs(sx) ** 2 * 0.4
    ...
    return { h, l, s, x: x * scale, y: y * scale, z: z * scale }
  }
